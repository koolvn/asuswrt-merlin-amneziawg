...
- **Профили конфигурации** -- до 5 клиентских конфигов с именами и переключением в один клик (v1.4.0); опциональное **автопереключение при отказе**: не прошедший проверку связности туннель по кругу пробует следующий профиль, перезагрузка/ручной выбор возвращают основной; из SSH — `amneziawg.sh profile [list|N|next]`
...
- **Приоритет над Xray** -- при активном XRAYUI в режиме «весь трафик» устройства с политикой **«VPN: весь трафик»** или гео-политикой (обычный, включающий режим) автоматически получают приоритет в туннель **впереди Xray** (цепочка `AWG_PRIO` в начале PREROUTING, с 1.3.12); остальная сеть — и гео в режиме исключения «Напрямую по спискам» — продолжает работать через Xray, правила Xray не трогаются
...
- **Настройка** — «Сгенерировать» пару ключей сервера, порт (по умолчанию `51821`, чтобы не конфликтовать со встроенным WG-сервером прошивки), подсеть туннеля (по умолчанию `10.9.0.0/24`), MTU. Параметры обфускации (`Jc/Jmin/Jmax/S1/S2/H1-H4`, кнопка «Сгенерировать случайные») + продвинутые сигнатурные пакеты `I1-I5` для AWG 2.x (тоже с кнопкой «Сгенерировать» — уникальная случайная подпись, маскирующая поток; встраивается в конфиг каждого пира).
...
- **Двойной хоп при работающем Xray — через галочку «мимо Xray»**: Xray перехватывает трафик раньше правил AmneziaWG (ip-rule 19 против 99), поэтому пир с VPN-политикой **без** этой галочки проваливается в Xray, а не в клиентский туннель — страница подсвечивает это красным баннером. Варианты: включить у пира «мимо Xray» (двойной хоп поверх работающего Xray, см. ниже), либо оставить пира на «Напрямую» (пойдёт через Xray), либо остановить Xray кнопкой в баннере. (У LAN-устройств этот приоритет включается сам — цепочка `AWG_PRIO`, 1.3.12; у пиров он намеренно отдан на откуп галочке «мимо Xray».)
...
- **Опция «мимо Xray» (per-peer, по умолчанию выкл)**: если нужен именно двойной хоп через клиентский туннель, а не проксирование через Xray — у пира с VPN-политикой есть галочка «мимо Xray». Она ставит правило перед перехватом Xray, уводя трафик этого пира в awg0 (клиентский туннель должен быть поднят). Остальные пиры продолжают ходить через Xray. Xray при этом работает и не перенастраивается.
...
Важно: это снимает только конфликт по DNS. При политике по умолчанию **«VPN -- весь трафик»** маршрутизация всё равно заберёт трафик у соседнего прокси, поэтому для совместимости выбирайте политику **«Напрямую»** или **«VPN -- только Geo»**, а не «весь трафик». Geo-маршрутизация по IP при этом продолжает работать. Это про политику **по умолчанию** — отдельным устройствам можно назначить AmneziaWG, и с 1.3.12 они получают приоритет в туннель даже при активном Xray (см. следующий пункт).

**Обратный случай — XRAYUI и трафик AmneziaWG.** Если XRAYUI работает в режиме прозрачного проксирования «перенаправить весь трафик» (TPROXY), он перехватывает LAN-трафик в mangle PREROUTING раньше правил AmneziaWG (ip-rule 19 против 98). Раньше это значило, что устройство, которому вы выбрали политику `VPN: весь трафик` или `VPN: только Geo`, по факту уходило в Xray, а не в туннель. **С 1.3.12 это исправляется автоматически:** пока Xray перехватывает трафик, аддон поднимает собственную цепочку `AWG_PRIO` в самом начале PREROUTING (перед XRAYUI) и даёт назначенным на AmneziaWG устройствам приоритет в туннель (кроме гео в режиме исключения «Напрямую по спискам» — он остаётся на Xray); остальная сеть продолжает ходить через Xray. Правила Xray при этом не трогаются. Страница показывает жёлтый информационный баннер с пояснением. **Остаётся один случай, который приоритет не решает:** если XRAYUI перехватывает и собственный egress роутера, включая рукопожатие AmneziaWG, туннель поднимается, но трафик не идёт (health-check откатывает его примерно через 60 с) — тогда исключите в XRAYUI endpoint и интерфейс `awg0` из перехвата, либо остановите Xray (кнопка в баннере), либо держите включённым только один VPN за раз.

**В: Что нужно, чтобы работал режим сервера (подключение к дому)?**

О: Белый (публичный) IP на WAN — или проброс UDP-порта аддона (по умолчанию 51821) на вышестоящем роутере. За CGNAT/серым адресом входящие подключения физически не дойдут — страница сервера предупредит красным баннером. Адрес для конфигов пиров подставляется автоматически (DDNS прошивки, иначе текущий WAN IP); клиентом на устройстве выступает официальное приложение **AmneziaWG** (сканирует QR со страницы). Автозапуск сервера после перезагрузки — отдельная галочка, по умолчанию выключена.

**В: Подключился к серверу роутера — рукопожатие есть, а у пира с политикой «VPN» нет интернета?**

О: Проверьте баннеры на странице сервера. Частые причины: клиентский туннель не запущен (политика «VPN» у пира — это двойной хоп через него; пока он лежит, трафик идёт напрямую — fail-open), либо работает Xray/XRAYUI в режиме «весь трафик» — тогда трафик пира с VPN-политикой Xray перехватывает раньше правил аддона. Чтобы всё-таки получить двойной хоп поверх работающего Xray, включите у пира галочку **«мимо Xray»** (клиентский туннель должен быть поднят); либо оставьте пира на «Напрямую» — его трафик и так пойдёт через Xray (страница подскажет, если Xray не подхватил подсеть пиров — лечится перезапуском XRAYUI).
